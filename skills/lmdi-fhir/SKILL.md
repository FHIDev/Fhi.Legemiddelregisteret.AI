---
name: lmdi-fhir
description: Kildebasert ekspert på LMDI-implementasjonsguiden (Legemiddeldata fra institusjon til Legemiddelregisteret). Bruk denne når brukeren stiller spørsmål om LMDI-profiler, hvordan ressurser skal fylles ut, ved validering av JSON mot profilene, for å lage eksempler, eller for å forstå bundle-struktur, extensions, valuesets, invariants og signert/kryptert innsending til LMR. Selvstendig — bygger på et kuratert øyeblikksbilde av LMDI-kildene i `references/` — versjon 1.1.5.
---
...
**Versjon**: IG `1.1.5` (FHIR `4.0.1`), bygd fra commit `02346e2a1`. Sjekk for nyere versjon ved behov — se §1.4.
...
1. Denne skillens `references/` — autoritativt grunnlag (øyeblikksbilde av LMDI 1.1.5).
...
- Ellers (valgfritt): hent fila fra raw-URL pinnet til 1.1.5 (se §1.4).

Finner du avvik mellom `references/` og kilden: **si det til brukeren** og følg `UPDATE.md`.

### 1.4 Kilder og versjonssjekk

`references/` er bygd fra LMDI 1.1.5 (commit `02346e2a1`). LMDI-repoet trengs ikke for å svare.

**Sjekk for nyere IG-versjon** (ved tvil eller på forespørsel) — hent `version:` fra kilden og sammenlign med `1.1.5`:

```
https://raw.githubusercontent.com/folkehelseinstituttet/LMDI/main/LMDI/sushi-config.yaml
```

Er versjonen på `main` nyere enn `1.1.5`: **fortell brukeren** at skillen kan være utdatert og bør oppdateres etter `UPDATE.md`.
...
```
https://raw.githubusercontent.com/folkehelseinstituttet/LMDI/02346e2a1cebc6509b1096709bdf91a7b3bdcc03/LMDI/input/fsh/<underkatalog>/<fil>.fsh
```
