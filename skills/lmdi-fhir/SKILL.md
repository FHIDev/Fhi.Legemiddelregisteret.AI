...
- **Skill tydelig mellom tre kildelag** i svaret:
  - `[LMDI-profilkrav]` — regel fra en FSH-fil i `LMDI/input/fsh/`.
  - `[LMDI-praksis]` — anbefaling fra `pagecontent/*.md` eller FSH-kommentarer (`^comment`, `^short`).
  - `[FHIR-basis, ikke LMDI-spesifikt]` — generell FHIR R4-regel som LMDI ikke eksplisitt dekker.
- Referer til konkrete filer og linjer (f.eks. `lmdi-Medication.fsh:491`).
...
1. FSH-filene i `LMDI/input/fsh/`.
2. `LMDI/sushi-config.yaml` og `LMDI/ig.ini`.
3. `LMDI/input/pagecontent/*.md` (intended use, protokoll, transport).
4. Genererte filer i `LMDI/fsh-generated/` eller `LMDI/output/` — bare når FSH ikke avgjør.
5. Denne skillens `references/` — avledet, verifiseres mot FSH ved tvil.

### 1.3 Stoppbetingelse før svar om strukturdetaljer

Gjelder spørsmålet kardinalitet, invariants, slicing, binding-strength eller `only Reference(...)`: **les den relevante FSH-filen først**. Ikke svar utelukkende fra referansefilene — de kan være utdaterte.

### 1.4 Sjekk av arbeidstre
...
1. **Åpne riktig FSH-fil**: `LMDI/input/fsh/profiles/lmdi-<Ressurs>.fsh` (normativ — ikke referansefilene).
...
- **Filsti + omtrentlig linje**: `lmdi-MedicationAdministration.fsh:233-236` (`time-required`-invarianten).
